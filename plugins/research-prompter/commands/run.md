...
1. **담당 셀의 조사 지시문 전문** — 브리프 "셀별 조사 지시문"(4-1) 절에서 해당 셀의
   문단을 **그대로** 복사해 넘긴다. **셀 라벨("A차원 × B관점")만 넘기지 않는다.**
   라벨만 받은 에이전트는 무엇을 어떻게 파야 할지 모르고 일반론을 돌려준다.
   이 전달이 조사 품질을 좌우한다.
2. 브리프의 배경·주요 연구 질문 요약
3. 해당 유형의 요구 분석 방식, 출처 요건

**브리프에 셀별 조사 지시문 절이 없으면** (구버전 브리프) 실행 전에 그 자리에서
매트릭스와 배경을 근거로 각 셀의 지시문을 생성한다. `:design` 4-1의 6요건
(인과관계·대안 비교·단계 분해·근거·출처·추론·예측·가정·한계)을 충족시킨다.
라벨만으로 에이전트를 띄우지 않는다.
