...
```
/research-prompter:design <리서치 주제>     # 성찰 → 매트릭스 → 셀별 조사 지시문 브리프
...
`:design`은 매트릭스만 그리고 끝내지 않습니다. **셀마다 완성된 조사 지시문을 씁니다.**

셀 라벨("국내 시공사별 × 생산성 실증 데이터")만으로는 조사 에이전트도 외부 딥리서치
도구도 무엇을 어떻게 파야 할지 모릅니다. 그래서 각 셀에 인과관계·대안 비교·단계 분해·
근거·추론·가정 6요건을 박은 3~6문장짜리 지시문을 생성하고, `:run`이 그것을 **그대로**
에이전트에 넘깁니다. 외부 딥리서치로 나갈 때도 이 지시문을 복사해 쓰면 됩니다.

매트릭스 자체도 템플릿 프리셋이 아니라 **넓은 관점 성찰**(주요 연구 질문 / 주제의 진화 /
관점·이해관계자 / 핵심 구성 요소)에서 도출합니다. 템플릿은 출발점이지 상한이 아닙니다 —
리서치 범위가 넓어지는 지점이 여기입니다.
...
```
docs/research/
├── briefs/BRIEF_<주제>_<날짜>.md       # 브리프 + 셀별 조사 지시문 + 검증 루프 2차 프롬프트
